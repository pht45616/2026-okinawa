...
* **💡 技術補充與限制注意點**：
  * **地圖瓦片 (Tiles) 快取**：Service Worker 可採用 Cache First 策略快取瀏覽過的 Leaflet 地圖瓦片。因瓦片數量龐大，無法預先快取未瀏覽區域，建議行前搭配 Google Maps 離線地圖作為備援。
  * **氣象預報 Fallback**：Open-Meteo 天氣 API 僅支援未來 7～14 天內預報。若出發日期尚遠，應具備 Fallback 機制（顯示該月份目的地歷史氣候均溫）。
  * **手機版雙模式切換技巧**：手機版實作「地圖模式」與「清單模式」切換（`display: none` / `block`）時，切換至地圖模式必須觸發 `map.invalidateSize()` 與 `map.fitBounds()`，否則 Leaflet 渲染會發生寬高異常或破圖。
